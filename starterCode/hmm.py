...
class HMM:
    __slots__ = ('pi', 'transitions', 'emissions',
                 'num_states', 'vocab_size', 'word_vocab')
    ...
    def predict_simple(self, sample, vocab_size, num_words_into_future):
        ...
        for i in range(num_words_into_future):
            ...
            print("simple: {} out of {} words complete".format(
                i+1, num_words_into_future))
        return pred
    ...
    def predict_with_viterbi(self, sample, num_words_into_future):
        ...
        for i in range(0, num_words_into_future):
            ...
            print("viterbi: {} out of {} words complete".format(
                i+1, num_words_into_future))
    ...
    if args.mode == 1:  # Mode for training
        ...
        for i in range(0, len(model_list)):
            ...
            test_LL[i] = model.loglikelihood_helper(
                dataset_flattened)/args.test_sample_size
            print("Tested: {} LL: {}".format(i, test_LL[i]))
        model.get_figure(range(1, len(test_LL)+1), test_LL,
                         'iterations', 'Log Likelihood per model', "test_LL")
